...
export class TableVisualizationClass
    ...
    private renderCell(headers: TableHeader[], columnIndex: number): (cellProps: CellProps) => JSX.Element {
        const { executionRequest, drillableItems, onFiredDrillEvent, rows, numberSeparator } = this.props;
        ...
        return (cellProps: CellProps) => {
            ...
            const { style, label } = getStyledLabel(header, cellContent, true, numberSeparator);
    ...
    private renderFooter(header: TableHeader, columnIndex: number, headersCount: number): JSX.Element {
        const { headers, totalsWithData, numberSeparator } = this.props;
