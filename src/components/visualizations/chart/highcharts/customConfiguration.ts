...
function formatLabel(value: any, format: any, config?: IChartConfig) {
    ...
    return escapeAngleBrackets(String(numberFormat(value, stripped, undefined, numberSeparator)));
}
...
function labelFormatterHeatMap(options: any, config?: IChartConfig) {
...
function getLabelsConfiguration(chartOptions: any, config?: IChartConfig) {
    ...
    return {
        ...
        plotOptions: {
            ...
            heatmap: {
                dataLabels: {
                    formatter: partial(labelFormatterHeatMap, config)
                }
            },
            treemap: {
